...
var Item = function () { this.intItemID = 0; }
var row_id = 1;
        ...
        for (let index = 0; index < response.length; index++) {
            ...
            if (itemQty > 0) {
                if (parseFloat(response[index].decAvailableQty) >= itemQty && itemQty > 0) {
                    ...
                    $(".first-tr").after('<tr class="' + itemID + '">' +
                        '<td hidden>' +
                        ...
                        '</td>' +
                        '<td class="static">' +
                        '<button type="button" class="button red center-items" onclick="removeItem(' + itemID + ')"><i class="fas fa-times"></i></button>' +
                        '</td>' +
                        '</tr>');
...
function removeItem(itemID) {
   var itemName = $("." + itemID).closest("tr").find('.itemName').val();
